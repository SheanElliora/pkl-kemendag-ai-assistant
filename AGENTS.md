...
- **RAG evaluation (update 01-09-2026 chunking adaptif live):** 16 pertanyaan diuji setelah retrieval tuning, chunking adaptif live, query expansion universal, fallback multi-tier. Hasil: 11 BENAR (68.8%), 3 SEBAGIAN (18.8%), 2 GAGAL/ERROR (12.5% Q15-Q16 Jurnal_Komoditas — instabilitas model, bukan retrieval). Fallback: minimax/minimax-m3:free → openai/gpt-4o-mini. Penyesuaian: (1) Chunking adaptif OTOMATIS via `chunkService.js:detectDocType()` + `getChunkConfig()` — peraturan 1600/400, akademik 2000/600, praktis 2000/500, general 2000/500 — dipanggil `pdfLoader.js:456` saat `processPDF`, version `v3-adaptive-512` di `chunkStorageService.js:51` (old array auto-rebuild), `bm25Service.js:122` backward-compat; total chunk 515 (was 631), vectors 515 di Chroma `sip_documents`; (2) Reranker `bge-reranker-base` MAX_LENGTH 512 (1024 menyebabkan Gather oob pada chunk 2400), `retrieverService.js` hybrid BM25_WIDTH 80 + RERANK_WIDTH 15; (3) Query expansion universal entity extraction (HS, USD/Rp, tahun, %) + LLM 15s cache; (4) `analyticsService.js` + `GET /api/stats` fallbackRate/modelUsage; (5) `llmService.js` multi-tier fallback + `generateAnswerStream` fallback, validasi empty choices, max_tokens 1536. Verifikasi 01-09-2026: `npm test` 14 PASS, `evalRag.mjs --no-llm` recall@7 = 7/7, `healthCheck --no-chat` backend OK.
