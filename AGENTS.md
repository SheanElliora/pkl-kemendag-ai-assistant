...
- **RAG evaluation (updated with fixes):** 16 pertanyaan sulit diuji (2 per dokumen x 8 dokumen) setelah penyesuaian retrieval parameters, TERM_EN expansion, dan reranker config. Hasil: 14 BENAR (87.5%), 2 GAGAL (backend model instability pada pertanyaan Jurnal_Komoditas, bukan retrieval issue). Peningkatan utama: (1) Q13 Jurnal_RAG index hot-swapping: jawaban "70% akurasi dengan indeks 2016" yang sebelumnya "Informasi tidak ditemukan" sekarang langsung menemukan & menampilkan sitasi [2], (2) peningkatan akurasi ekstraksi angka spesifik melalui keyword bonus 0.1, (3) reranker MAX_LENGTH dinaik dari 512 ke 1024 untuk mencegah truncasi konteks kritis di akhir chunk, (4) penambahan 9 istilah teknis ke kamus TERM_EN untuk ekspansi query Indonesia→Inggris, (5) peningkatan robustibilitas LLM dengan validasi empty choices dan max_tokens 1536.
