...
- **RAG evaluation (update multi-tier fallback):** 16 pertanyaan diuji setelah penyesuaian retrieval parameters, chunking adaptif, query expansion universal, dan fallbacks model AI. Hasil: 11 BENAR (68.8%), 3 SEBAGIAN (18.8%), 2 GAGAL/ERROR (12.5% - ketidakstabilan model AI Q15-Q16 Jurnal_Komoditas, bukan retrieval issue). Sistem sudah dilengkapi fallback multi-tier: minimax/minimax-m3:free → openai/gpt-4o-mini → fallback lebih cerdas. Penyesuaian utama: (1) MAX_LENGTH reranker 512→1024 mencegah truncasi konteks, (2) Query expansion universal dengan entity extraction (HS code, angka, nilai USD, entitas), (3) Chunking adaptif per tipe dokumen (akademik/peraturan/praktis/general), (4) Sistem analytics melacak model usage, fallback rate, dan kategori pertanyaan, (5) validasi empty choices di LLM response.
