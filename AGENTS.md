...
- **RAG evaluation (update 01-09-2026 chunking adaptif live + model gratis):** 16 pertanyaan diuji setelah retrieval tuning, chunking adaptif live, query expansion universal, fallback multi-tier gratis. Hasil: 11 BENAR (68.8%), 3 SEBAGIAN (18.8%), 2 GAGAL/ERROR (12.5% Q15-Q16 Jurnal_Komoditas — instabilitas model, bukan retrieval). Fallback gratis: `google/gemini-2.0-flash-exp:free` → `minimax/minimax-m3:free` → `deepseek/deepseek-chat:free` → `openai/gpt-4o-mini` (berbayar fallback terakhir). Penyesuaian: (1) Chunking adaptif OTOMATIS via `chunkService.js:detectDocType()` + `getChunkConfig()` — peraturan 1600/400, akademik 2000/600, praktis 2000/500 — dipanggil `pdfLoader.js:456`, version `v3-adaptive-512` di `chunkStorageService.js:51`, `bm25Service.js:122` backward-compat; total chunk 515 (was 631), vectors 515 di Chroma `sip_documents`; (2) Reranker `bge-reranker-base` MAX_LENGTH 512 (1024 → Gather oob), `retrieverService.js` hybrid BM25_WIDTH 80 + RERANK_WIDTH 15; (3) Query expansion universal entity extraction + LLM 15s cache; (4) `analyticsService.js` + `GET /api/stats`; (5) `llmService.js` multi-tier fallback gratis + `generateAnswerStream` fallback, max_tokens 1536. Verifikasi 01-09-2026: `npm test` 14 PASS, `evalRag.mjs --no-llm` recall@7 = 7/7, `healthCheck --no-chat` 3 PASS (backend OK, Chroma 515 vectors ≥400). Tambahan 01-09-2026 gratis-optimal: `pdfLoader.js:28` MIN_DIGITAL_CHARS 500→1200 + `meaningfulPages≥3` cegah scan tabel tipis lolos digital; `healthCheck.mjs:33` MIN_VECTORS 600→400 (akurat untuk 515); `data/query_expansion_cache.json` duplikat root dihapus.
